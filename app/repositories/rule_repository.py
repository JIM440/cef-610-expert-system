...
def get_active_rules_for_crop(crop_id: int) -> list[dict]:
    return fetch_all(
        ...
        SELECT r.id AS rule_id, r.rule_name, r.confidence_score, r.is_active,
               ...
               re.condition_value_id, ec.name AS condition_name,
               ecv.value_name AS condition_value, re.is_required AS condition_required
        FROM rule r
...
def get_all_rules() -> list[dict]:
    return fetch_all(
        ...
        SELECT r.id, r.rule_name, r.confidence_score, r.disease_id,
               ...
               COALESCE((SELECT string_agg(ec.name || ': ' || ecv.value_name, ', ' ORDER BY ec.name)
                         FROM rule_environment re
                         JOIN environmental_condition_value ecv ON ecv.id = re.condition_value_id
...
def get_rule_detail(rule_id: int) -> dict | None:
    ...
    result["condition_value_ids"] = [r["condition_value_id"] for r in fetch_all("SELECT condition_value_id FROM rule_environment WHERE rule_id = %(id)s ORDER BY condition_value_id", {"id": rule_id})]
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            for value_id in condition_value_ids:
                cur.execute("INSERT INTO rule_environment (rule_id, condition_value_id, is_required) VALUES (%s, %s, TRUE)", (rule_id, value_id))
...
def link_rule_environment(rule_id: int, condition_value_id: int) -> None:
    execute("INSERT INTO rule_environment (rule_id, condition_value_id, is_required) VALUES (%(rule_id)s, %(value_id)s, TRUE) ON CONFLICT (rule_id, condition_value_id) DO NOTHING", {"rule_id": rule_id, "value_id": condition_value_id})
